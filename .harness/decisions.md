...
## D-013 — Setup do ciclo vira fail-closed; runtime e diagnostico continuam intocados (2026-08-16)
Decisão: compile-contract, compile-session, harness verify e harness supervise recusam (exit 1) em vez de avisar quando falta harness.yaml (setup) ou falta enforcement nesta maquina com contrato ativo (trabalho); runtime (escapes do boundary_guard: task add-file, .harness/scratch/, YAML colavel do deny) e diagnostico (status/doctor/health) continuam intocados, so reportando
Porquê: Reverte deliberadamente o nao-objetivo do contrato governanca-parcial-invisivel-sem-init (nao tornar harness.yaml obrigatorio para compile-session, spec.md:73-74 daquele contrato, v0.30.0) -- o aviso em stderr missing_harness_yaml_warning provou-se invisivel: numa sessao real o preflight rodou, o plan rodou sem harness.yaml, o aviso passou despercebido, o plano executou inteiro com alteracoes fora do contrato passando, e o harness so confessou a governanca ausente no fim, tarde demais para importar. Racional da distincao: o harness barra o minimo (harness-deve-barrar-o-minimo) e criterio de RUNTIME -- agente sozinho por horas, deny duro sem escape empurra para o kill-switch, que e desprotecao total, pior que o deny que tentava evitar; setup e outro tempo -- humano presente na tela, uma vez por projeto, custo de destravar e rodar /harness-creator:init. Escopo decidido na implementacao do T-03: enforcement_gate_problem (health.py) so dispara em repo GOVERNADO (harness.yaml presente) com contrato ativo (feature_list.json presente) -- repo sem harness.yaml e caso do gate de setup (T-01/T-02, GovernanceNotInstalledError/require_harness_yaml_installed), nao deste gate; misturar os dois faria verify/supervise competir por mensagem alheia e quebraria a suite de testes de run_verify/dispatch_next, que roda de proposito com contrato ativo e sem hooks/settings instalados (logica, nao governanca).
