...
Se a chave ja existe, basta acrescentar a linha `    - python scripts/deploy.py` na lista. Vale na tool call SEGUINTE — o guard le esse arquivo a cada chamada, sem recompilar nada. E casa por PREFIXO: essa entrada libera o comando com qualquer argumento depois dela. (3) Replaneje via /harness-creator:plan so se o ESCOPO da tarefa mudou.`
