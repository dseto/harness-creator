...
Gerado em 2026-07-16T11:31:59.502807+00:00 por `tests/e2e/test_fase3_outcomes.py` (repos sintéticos em tmp_path via subprocess da CLI real + hooks standalone gerados + funções públicas de módulo; o outcome 6 verifica a evidência do dogfood real já gravada em disco).
...
```json
{
  ...
  "recorded_at": "2026-07-16T11:31:53.265271+00:00",
  "exit_code": 0,
...
_Atualizado em 2026-07-16T11:31:59.502807+00:00 por esta rodada._
...
## Outcome 6 — evidência do dogfood real da Fase 3 em disco completa (permission_denials não-vazio; passes:true só após evidência real); evidências das Fases 1/2 intactas

Veredito: **ATINGIDO**

`fase3-dogfood-verify-lock.md` existe com as 5 seções esperadas: ['## Regressão (Fases 1/2 na mesma cobaia)', '## Verify real (harness verify T-01)', '## Feature-lock (negação prematura + permissão legítima)', '## Diff aplicado (T-01)', '## Execução do agente'].
Campo estruturado `permission_denials` parseado da evidência: 1 negação(ões); a primeira é Edit sobre `feature_list.json` com old_string `"passes": false` -> new_string `"passes": true` — a negação registrada é exatamente a tentativa prematura de marcar done sem evidência, com `is_error: False` (a sessão real terminou bem mesmo assim).
A seção do verify real mostra a evidência gravada com `"exit_code": 0` e a confirmação de que o `files_hash` recalculado FORA do Claude bate com o gravado.
Estado final `passes: true` confirmado com a ordem temporal provada fora do Claude: `mtime feature_list.json (1784200282.776518) >= mtime evidência (1784200275.101763): confirmado` — a lista só foi marcada done depois da evidência real.
`fase1-dogfood-document-digits.md` (TDD real Document_with_letters_fails) e `fase2-dogfood-boundary-guard.md` (negação fora do raio com permission_denials estruturado) continuam intactas em disco — zero regressão de evidência entre fases.
